...
def solveMaze(original_binary_img, initial_point, final_point, no_cells_height, no_cells_width):
    ...
    queue = []
    parent = [[0 for i in range(no_cells_height)]
    ...
    while(len(queue) != 0):
        x, y = queue.pop(0)  # down
        for (i, j) in dir1:
            tempx, tempy = int(((x+0.5)*CELL_SIZE)), int(((y+0.5)*CELL_SIZE))
            desx, desy = int(((x+i+0.5)*CELL_SIZE)), int(((y+j+0.5)*CELL_SIZE))
            if(y+j >= 0 and y+j <= no_cells_height-1 and x+i >= 0 and x+i <= no_cells_width-1 and visited[x+i][y+j] == 0 and (a[((tempx+desx)//2)-1][((tempy+desy)//2)-1] == 255)):
                visited[x+i][y+j] = 1
                queue.append((x+i, y+j))
                parent[x+i][y+j] = (x, y)
                cv2.rectangle(img, ((y+j)*20+3, (x+i)*20+3),
                              ((y+j)*20-3+20, (x+i)*20-3+20), (128, 128, 128), -1)
    ...
    if found == 1:
        while(1):
            ...
            cv2.rectangle(img, ((goaly)*20+2, (goalx)*20+2),
                          ((goaly)*20-2+20, (goalx)*20-2+20), (255, 128, 0), -1)
            time.sleep(0.005)
        ...
        cv2.rectangle(img, ((startx)*20+3, (starty)*20+3),
                      ((startx)*20-3+20, (starty)*20-3+20), (255, 128, 0), -1)
...
while p < 2:
    pass
z = solveMaze(a, start, end, img.shape[0]//20, img.shape[0]//20)
print(z)
cv2.waitKey(0)
